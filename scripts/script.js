...
function renderJobs() {
    ...
    filtered.forEach(job => {
        ...
        div.innerHTML =
            `
    <button onclick = 'deleteJob(${job.id})' class = 'absolute top-4 right-4 text-gray-400 hover:text-red-500 text-lg'><i class="fa-regular fa-trash-can"></i></button>
    ...
    <div class="flex gap-3">
        <button onclick="updateStatus(${job.id}, 'interview')"
            class="bg-emerald-100 text-emerald-600 text-xs px-3 py-1 rounded hover:bg-emerald-200 transition"> INTERVIEW
        </button>


        <button onclick="updateStatus(${job.id}, 'rejected')"
            class="bg-red-100 text-red-600 text-xs px-3 py-1 rounded hover:bg-red-200 transition"> REJECTED
        </button>
    </div>
...
function deleteJob(id){
    const index = jobs.findIndex(j => j.id);
    jobs.splice(index,1);
    renderJobs();
}
...
function updateDashboard() {
    totalCount.innerText = jobs.length; // Total jobs
    interviewCount.innerText = jobs.filter(j => j.status === "interview").length; // Interview jobs
    rejectedCount.innerText = jobs.filter(j => j.status === "rejected").length; // Rejected jobs
}
